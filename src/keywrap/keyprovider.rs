...
#[derive(Debug)]
pub struct KeyProviderKeyWrapper<'b> {
    pub provider: &'b str,
    pub attrs: &'b KeyProviderAttrs,
    pub runner: Option<Box<dyn utils::CommandExecuter>>
}
...
#[derive(Serialize, Deserialize, Debug)]
pub struct KeyWrapParams<'a> {
    pub ec: Option<&'a EncryptConfig>,
    optsdata: Vec<u8>,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct KeyUnwrapParams<'a> {
    pub dc: Option<&'a DecryptConfig>,
    annotation: Vec<u8>,
}
...
pub fn new_key_wrapper<'b>(p: &'b str, kp_conf: &'b KeyProviderAttrs, runner: Option<Box<dyn utils::CommandExecuter>>) -> &'b KeyProviderKeyWrapper<'b>{
...
impl KeyWrapper for KeyProviderKeyWrapper<'_> {
    ...
    fn wrap_keys(&self, enc_config: &EncryptConfig, opts_data: &[u8]) -> Result<Vec<u8>> {
        ...
        let key_wrap_params = KeyWrapParams { ec: Some(enc_config), optsdata: Vec::from(opts_data) };
        ...
        if enc_config.param.contains_key(&self.provider.to_string()) {
            if &self.attrs.cmd.path != "" {
                protocol_output =  get_provider_command_output(serialized_input, &self.attrs.cmd, &self.runner).unwrap()
            } else if &self.attrs.grpc != "" {
                let mut rt = Runtime::new().unwrap().block_on(get_provider_grpc_output(serialized_input, &self.attrs.grpc, OP_KEY_WRAP));
                protocol_output = rt.unwrap()
            }
        }
        Ok(protocol_output.keywrapresults.unwrap().annotation)
    }

   fn unwrap_keys(&self, dc_config: &DecryptConfig, json_string: &[u8]) -> Result<Vec<u8>> {
        let mut protocol_output = KeyProviderKeyWrapProtocolOutput { keywrapresults: None, keyunwrapresults: None };
        let key_unwrap_params = KeyUnwrapParams { dc: Some(dc_config), annotation: Vec::from(json_string) };
        let input = KeyProviderKeyWrapProtocolInput {
        ...
        if &self.attrs.cmd.path != "" {
            ...
            let rt = Runtime::new().unwrap().block_on(get_provider_grpc_output(serialized_input, &self.attrs.grpc, OP_KEY_UNWRAP));
            let rt = Runtime::new().unwrap().block_on(get_provider_grpc_output(serialized_input, &self.attrs.grpc, OP_KEY_UNWRAP));
            protocol_output = rt.unwrap();
        }
    ...
    fn annotation_id(&self) -> &str {
        let meta = "org.opencontainers.image.enc.keys.provider.";
        &format!("{}{}", meta.to_string(), self.provider.to_string())
    }
...
async fn get_provider_grpc_output(input: Vec<u8>, connection_string: &'static str, operation: &str) -> Result<KeyProviderKeyWrapProtocolOutput> {
    let mut protocol_output = KeyProviderKeyWrapProtocolOutput { keywrapresults: None, keyunwrapresults: None };
...
fn get_provider_command_output(input: Vec<u8>, cmd: &Command, runner: &dyn utils::CommandExecuter) ->Result<KeyProviderKeyWrapProtocolOutput> {
    let mut protocol_output = KeyProviderKeyWrapProtocolOutput { keywrapresults: None, keyunwrapresults: None };
    let resp_bytes: Vec<u8>;
    // Convert interface to command structure
    unsafe {
        resp_bytes =  runner.exec((*cmd.path).to_string(), &cmd.args, input).unwrap();
    }
    protocol_output = bincode::deserialize(&resp_bytes).unwrap();
    Ok(protocol_output)
}
...
mod tests {
    ...
    impl CommandExecuter for TestRunner {
        ...
        fn exec(&self, cmd: String, args: &Vec<String>, input: Vec<u8>) -> std::io::Result<Vec<u8>> {
            ...
            if cmd == "/usr/lib/keyprovider1-wrapkey" {
            ...
            } else if cmd == "/usr/lib/keyprovider1-unwrapkey" {
                let keyp: KeyProviderKeyWrapProtocolInput = bincode::deserialize(input.as_ref()).unwrap();
    ...
    fn test_key_provider_command_success() {
        ...
        let kp = config::get_configuration().unwrap().key_providers;
        let key_wrapper = new_key_wrapper("keyprovider", kp.get("keyprovider").unwrap(), Some(Box::new(test_runner)));

        let mut ec = EncryptConfig::default();
        let mut dc = DecryptConfig::default();
        let mut ec_params = vec![];
        ec_params.push(from_slice(b"keyprovider").unwrap());
        ...
        let opts_data = b"symkey";

        let kp_attrs = config::get_configuration().unwrap().key_providers.get("keyprovider").unwrap();
        let key_wrapper = new_key_wrapper("keyprovider", kp_attrs, Some(Box::new(test_runner)));

        let mut dc_params = vec![];
        dc_params.push(from_slice(b"keyprovider").unwrap());

        assert!(dc.decrypt_with_key_provider(dc_params).is_ok());

        assert!(key_wrapper.unwrap_keys(&dc, key_wrap_output_result.unwrap().as_ref()).is_ok());
    ...
    fn test_key_provider_grpc_success() {
        ...
        let opts_data = b"symmetric_key";
        let oci_config = config::get_configuration().unwrap();
        let mut ec = EncryptConfig::default();
        let mut dc = DecryptConfig::default();
        ...
        let key_wrapper = new_key_wrapper("keyprovider1", oci_config.key_providers.get("keyprovider1").unwrap(), None);
        assert!(ec.encrypt_with_key_provider(ec_params).is_ok());
